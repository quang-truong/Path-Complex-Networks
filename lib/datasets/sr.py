...
from lib.utils.graph_to_complex import convert_graph_dataset_with_rings, convert_graph_dataset_with_paths

from lib.utils.log_utils import makedirs
from lib.utils.sr_utils import load_sr_dataset
...
class SRDataset(InMemoryComplexDataset):
    ...
    def __init__(self, root, name, max_dim=2, num_classes=16, train_ids=None, val_ids=None, test_ids=None, 
                 include_down_adj=False, max_ring_size=None, n_jobs=2, init_method='sum', complex_type='path', **kwargs):
        self.name = name
        ...
        super(SRDataset, self).__init__(root, max_dim=max_dim, num_classes=num_classes,
            include_down_adj=include_down_adj, complex_type=complex_type, init_method=init_method)
    ...
    def process(self):
        ...
        graphs, _, _, _ = load_sr_graph_dataset(self.name, self.root, prefer_pkl=True)
        if self._complex_type == 'cell':
            print(f"Converting the {self.name} dataset to a cell complex...")
            complexes, max_dim, _ = convert_graph_dataset_with_rings(
                graphs,
            ...
            print(f"Converting the {self.name} dataset to a path complex...")
...
def load_sr_graph_dataset(name, root, prefer_pkl=False):
