...
def check_edge_index_are_the_same(upper_index, edge_index):
    """Checks that two edge/cell indexes are the same.
        upper_index: data1.cochian[0].upper_index from dataset1 (complex dataset)
        edge_index: data2.edge_index from dataset2 (graph dataset)
    """
...
def get_table(boundary_index):
    ...
    num_elements = boundary_index.size(1)
    id_to_cell = dict()
...
def check_edge_attr_are_the_same(boundary_index, e_x, edge_index, edge_attr):
    """Checks that a pairs of edge attributes are identical.
        boundary_index and e_x: data1.cochains[1].boundary_index and data1.cochains[1].x from dataset1 (complex dataset)
        edge_index and edge_attr: data2.edge_index and data2.edge_attr from dataset2 (graph dataset)
    """
    ...
    num_edges = boundary_index.size(1)
    for i in range(num_edges):
        e1, e2 = edge_index[0, i].item(), edge_index[1, i].item()           # get two ends of an edge from edge_index of grpah
        edge = tuple(sorted([e1, e2]))                                      # sort e1 and e2

        edge_attr1 = e_x[edge_to_id[edge]].squeeze()                        # convert sorted([e1, e2]) back to index, then get feature e_x[index].squeeze()
        edge_attr2 = edge_attr[i].squeeze()                                 # get feature based on PytorchGeometric edge_attr interface
        
        # NB: edge feats may be multidimensional, so we cannot
        # generally use the `==` operator here
        assert torch.equal(edge_attr1, edge_attr2)                          # check if they are equal


def get_induced_rings_by_nx(n, edge_index, max_ring):
...
def get_complex_rings(r_boundary_index, e_boundary_index):
    """Extracts the vertices that are part of a ring.
        Used when we are sure that complex.cochains has key 2 (2-cochain exists)
    """
    ...
    rings = set()
    for ring, edges in id_to_ring.items():              # iterate through dict of ring: [edges]
